...
\hypersetup{
  pdftitle={Diagnosing Reward Misspecification and Observation Insufficiency},
  pdfauthor={Emilio Daza, Jacob Lee, and Rohin Desai}
}
...
\graphicspath{{../artifacts/five_seed/}}
...
The implementation stores $S_t$, $R_t^\star$, and $\widetilde R_t$
separately, exposing only the latter in passive logs. Ground-truth labels live
in a separate manifest keyed by opaque run identifiers. The following oracle
...
\paragraph{Controlled environments.}
The factorial benchmark has two latent states and two actions, horizon $H=5$,
and $\Pp(S=0)=0.75$. The intended reward is
$R^\star=\ind\{A=S\}$. Observation corruption maps both states to a sentinel;
reward corruption changes the proxy target to $1-S$. The four independent
toggle settings realize neither, reward-only, observation-only, and both
failures. A second benchmark implements the exact two-system construction in
the proposition. It is essential: the factorial benchmark is a controlled
positive task, whereas only the matched pair tests passive impossibility.

\paragraph{Learners, logging, and probes.}
We ran tabular Q-learning and clipped tabular PPO for 1,000 transitions using
five paired seeds in each condition. Across four factorial conditions and two
matched controls this gives 60 runs and 60,000 passive transitions. Passive
rows contain observations, actions, proxy rewards, action probabilities,
entropy, value estimates, TD errors, boundaries, steps, and seeds; neither
latent state, intended reward, nor class label is present. State, reward-audit,
and combined probes use balanced actions and $4,8,16,32,$ or $64$ probe
episodes. Prices are $c_E=1$ and $c_S=c_R=4$.

\paragraph{Diagnostics and splits.}
We extract the first $5\%$, $10\%$, and $20\%$ of every run. Baselines use
random guesses, reward-curve summaries, or standard training statistics
(entropy, action diversity, observation coverage, TD error, episode length,
and a held-out history-dependence score). Logistic diagnostic models use
leave-one-paired-seed-out splits, so no transition from a held-out trajectory
enters training. We report balanced accuracy, macro-F1, class recall, confusion
matrices, and paired-seed bootstrap intervals.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{rl_episode_snapshot.png}
\caption{A frame from the reproducible 2D playback. The learner sees the
sentinel observation but the evaluator separately records latent state and
intended reward. Here the proxy rewards action 1 even though intended reward is
zero, directly visualizing the ``both'' condition.}
\label{fig:playback}
\end{figure}
...
\centering
\small
\caption{Balanced diagnostic accuracy at the $20\%$ prefix. Probe columns use
64 episodes. Chance is $0.25$ for factorial diagnosis and $0.50$ for the matched
binary control.}
\label{tab:main-results}
\begin{tabular}{llccccc}
\toprule
Learner & Benchmark & Random & Passive stats & State & Reward audit & Combined\\
\midrule
Q-learning & Factorial & 0.35 & 0.45 & 1.00 & 0.80 & 1.00\\
Q-learning & Matched   & 0.30 & 0.40 & 0.90 & 0.80 & 1.00\\
PPO        & Factorial & 0.35 & 0.70 & 1.00 & 0.95 & 1.00\\
PPO        & Matched   & 0.30 & 0.50 & 1.00 & 1.00 & 1.00\\
\bottomrule
\end{tabular}
\end{table*}

\paragraph{Ground-truth gaps.}
With $q=0.75$ and $H=5$, the factorial reward gaps are $0,5,0,$ and $2.5$ for
neither, reward-only, observation-only, and both; observation gaps are
$0,0,1.25,$ and $1.25$. The matched observation system has gaps $(0,1.25)$ and
the matched reward system $(5,0)$. The memory gap is zero throughout: useful
information is either current or intrinsically absent. Every reward has binary
support $[0,1]$, ruling out reward scale as a shortcut.

\paragraph{Passive impossibility check.}
The empirical proxy-reward rates conditioned on action agree closely across
the two matched mechanisms. For PPO, the absolute differences are $0.0045$ for
action 0 and $0.0023$ for action 1; for Q-learning they are $0.0092$ and
$0.0113$. This is sampling fluctuation around exact equality in population.
Accordingly, passive standard-statistics accuracy on the matched control is
$0.50$ for PPO and $0.40$ for Q-learning. Finite-sample scores above or below
chance do not contradict non-identifiability.

\paragraph{Interventions and cost.}
Combined probes reach perfect balanced accuracy in all four learner--benchmark
combinations at 64 episodes. Using the conservative rule that the 95\% bootstrap
lower bound must be at least $0.90$, the cheapest tested intervention at the
$20\%$ prefix costs 20 for both factorial learners (four state or reward probe
episodes), 40 for PPO on the matched control (eight reward audits), and 72 for
Q-learning on the matched control (eight combined probes). These minima are
specific to this candidate family, cost vector, and five-seed sample.

\begin{figure*}[t]
\centering
\begin{minipage}{0.49\textwidth}\includegraphics[width=\linewidth]{accuracy_prefix_factorial_tabular.png}\end{minipage}
\begin{minipage}{0.49\textwidth}\includegraphics[width=\linewidth]{accuracy_prefix_factorial_ppo.png}\end{minipage}\\[-0.3em]
\begin{minipage}{0.49\textwidth}\includegraphics[width=\linewidth]{accuracy_prefix_matched_tabular.png}\end{minipage}
\begin{minipage}{0.49\textwidth}\includegraphics[width=\linewidth]{accuracy_prefix_matched_ppo.png}\end{minipage}
\caption{Balanced accuracy against passive-prefix length. The matched control
keeps standard passive diagnosis near chance, while targeted probes separate
the mechanisms. Probe curves shown here use 64 episodes.}
\label{fig:prefix}
\end{figure*}

\begin{figure*}[t]
\centering
\begin{minipage}{0.49\textwidth}\includegraphics[width=\linewidth]{accuracy_cost_factorial_tabular.png}\end{minipage}
\begin{minipage}{0.49\textwidth}\includegraphics[width=\linewidth]{accuracy_cost_factorial_ppo.png}\end{minipage}\\[-0.3em]
\begin{minipage}{0.49\textwidth}\includegraphics[width=\linewidth]{accuracy_cost_matched_tabular.png}\end{minipage}
\begin{minipage}{0.49\textwidth}\includegraphics[width=\linewidth]{accuracy_cost_matched_ppo.png}\end{minipage}
\caption{Accuracy--cost frontiers at the $20\%$ prefix. More data is not
monotonically better in this small sample because each point retrains a finite
diagnostic model; the conservative minimum-cost table therefore uses interval
lower bounds rather than selecting by point estimate alone.}
\label{fig:cost}
\end{figure*}

\begin{figure*}[t]
\centering
\begin{minipage}{0.49\textwidth}\includegraphics[width=\linewidth]{confusion_factorial_tabular.png}\end{minipage}
\begin{minipage}{0.49\textwidth}\includegraphics[width=\linewidth]{confusion_factorial_ppo.png}\end{minipage}\\[-0.3em]
\begin{minipage}{0.49\textwidth}\includegraphics[width=\linewidth]{confusion_matched_tabular.png}\end{minipage}
\begin{minipage}{0.49\textwidth}\includegraphics[width=\linewidth]{confusion_matched_ppo.png}\end{minipage}
\caption{Leave-one-seed-out confusion matrices at the $20\%$ prefix. Each panel
contrasts standard passive statistics (left subpanel) with the 64-episode
combined probe (right subpanel).}
\label{fig:confusions}
\end{figure*}

\clearpage

\section{Shortcuts, confounds, and reproducibility}

The factorial benchmark intentionally exposes observation identity and
coverage, so it should not be interpreted as proving unrestricted passive
identification. The matched control removes that shortcut. A state-only probe
can solve this structured family because the proxy--state relation is itself
diagnostic; unrestricted four-class guarantees still require A1--A5 and may
require both probes. The history-dependence feature is predictive association,
not a causal proof of intrinsic missing information. Finally, five seeds are
appropriate for pipeline validation, not precise scientific claims.

The repository includes the environment and learner implementations, strict
passive schema, raw passive and privileged logs, probe logs, run manifest,
oracle gaps, all diagnostic metrics and confidence intervals, minimum-cost
table, figures, animation, tests, and this source. The full experiment is
reproduced by \texttt{python -m identirl run}; the playback is produced by
\texttt{python -m identirl animate}. Eleven automated tests cover Gymnasium
compatibility, factorial semantics, passive equivalence, value gaps, schema
isolation, both learners, interventions, prefix extraction, and seed-level
evaluation.
